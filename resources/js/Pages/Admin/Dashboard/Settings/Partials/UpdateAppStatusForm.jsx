...
export default function UpdateAppStatusForm({
  className = '',
  success,
  message
}) {
  ...
  const { data, setData, post, errors, processing, recentlySuccessful } = useForm({
    app_status: message.app_status,
    _method: 'PUT'
  });

  const submit = (e) => {
    e.preventDefault();
    post(route('settings.update.appstatus', message.id));
  };

  return (
    <section className={className}>
      <header>
        <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">حالة التطبيق</h2>
      </header>
