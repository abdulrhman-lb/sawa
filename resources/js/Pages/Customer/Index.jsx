...
import { FaBorderNone, FaRegEdit, FaRegPlusSquare, FaUserTie } from "react-icons/fa";
...
export default function index({
  ...
  const addPayment = (customer) => {
    router.get(route("customer.payment", {'id': customer.id}))
  }
                  ...
                  <tbody className="text-center">
                    {customers.data.map((customer, index) => (
                      <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={customer.id}>
                        ...
                        <td className="px-3 py-2 text-red-800">{(customer.amount_kind_id === null) && "يجب تثبيت السرعة"} {(customer.phone === null) && "يجب تثبيت رقم الهاتف"}</td>
                        <td className="px-3 py-2 text-nowrap">
                          <div className="flex justify-center">
                            <button
                              onClick={e => editCustomer(customer)}
                              type="button"
                            ...
                            <button
                              onClick={e => addPayment(customer)}
                              type="button"
                              disabled={(customer.amount_kind_id === null || customer.phone === null) ? true : false}
                              className="inline-flex text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 shadow-lg shadow-blue-500/50 dark:shadow-lg dark:shadow-blue-800/80 font-medium rounded-lg text-lg px-2.5 py-1.5 text-center me-2">
                              دفع فاتورة
                              <MdPayment style={{ marginRight: '8px', marginTop: '2px' }} size={25} />
                            </button>
                          </div>
                        </td>
                      </tr>
