...
export default function SearchableDropdownCenter({ items, name, queryParams, selectedItem, onSelectItem, placeholder = "اختر عنصرًا", change }) {
...
const handleSelectItem = (item) => {
  ...
  change(item.id); // استدعاء changeCustomer بعد اختيار العنصر
  if (item.id === 0) { // إذا تم اختيار "عرض الكل"
...
const dropdownMenu = (
  ...
  return (
    <div className="relative w-44 text-nowrap" ref={dropdownRef}>
      <button
        type="button"
        onClick={toggleDropdown}
        className="w-full h-10 text-sm flex items-center p-2 text-right border bg-white border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white"
      >
